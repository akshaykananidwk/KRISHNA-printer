...
| | |
|---|---|
| Run at | 2026-09-13 14:41:40 UTC |
...
| T23.5 | Secrets are encrypted at rest | ciphertext, not the plaintext value | Stored as sod1:aKbwQJs… — the plaintext does not appear in the database. | **PASS** |
...
| T2.2 | The QR token is stored only as a hash | no plaintext token in the database | Only the SHA-256 hash is stored (hint: a14977b4…). A stolen database dump yields no working QR link. | **PASS** |
...
| T5.4 | Stored files get a random name, not the uploaded one | no original filename on disk | Original "twelve-pages.pdf" stored as "570ee2bdadf401847edb580f3962d2348b5bc98f.pdf" — the uploaded name never touches the filesystem path. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T17.1 | A job is created and queued | a job number is issued | Job created; payment_required=false, redirect=/print/success/dfb5e45687f6e94458c606c99e8a0720 | **PASS** |
...
| T17.2 | The worker sends the job to the printer over IPP | the printer accepts it | Status "completed", printer job id 6. Worker output: 14:41:34 [sent] AK100001 — Printer accepted the job (IPP job 6). \| 14:41:34 Reconciled: 1 completed, 0 failed of 1 checked \| 14:41:34 Worker finished: 1 job(s) in 0 second(s) | **PASS** |
...
| T10.1 | A4 printing works end to end | the job completed on A4 | Completed on A4 at 2026-09-13 14:41:34 UTC. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T25.1 | A database backup is created and is restorable | a verified, non-trivial dump | Backup db-20260913-144136-0fe46875.sql.gz: 11,747 bytes, checksum verified, contains CREATE TABLE for print_jobs. | **PASS** |
| T25.2 | An application backup is created | a zip containing the code | Application backup app-20260913-144136-0e765b90.zip contains the code and correctly excludes customer uploads. | **PASS** |
...
| T29.3 | The QR code is genuinely scannable | decodes back to the print URL | Version 7 QR (45×45 modules) rendered as a 2,399 byte PNG. Independently decoded with OpenCV during development. | **PASS** |
...
| T30.3 | The location list pages rather than loading everything | bounded page size | 25 of 52 locations rendered (paged at 25) in 8 ms. | **PASS** |
| T30.4 | Health checks across 50+ printers complete promptly | a bounded batch | Completed in 58 ms — 14:41:39 [printer_health] checked 0 — 0 online, 0 not (2 ms) | **PASS** |
...
_Generated by `tests/run.php` on 2026-09-13 14:41:40 UTC. Re-run with `php tests/run.php`._
