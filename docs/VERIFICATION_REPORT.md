...
| | |
|---|---|
| Run at | 2026-09-13 20:13:01 UTC |
...
| T23.5 | Secrets are encrypted at rest | ciphertext, not the plaintext value | Stored as sod1:a4UueqR… — the plaintext does not appear in the database. | **PASS** |
...
| T2.2 | The QR token is stored only as a hash | no plaintext token in the database | Only the SHA-256 hash is stored (hint: b07fb0f7…). A stolen database dump yields no working QR link. | **PASS** |
...
| T5.4 | Stored files get a random name, not the uploaded one | no original filename on disk | Original "twelve-pages.pdf" stored as "c890b015bb031ff3b5a27e6351cb09e30ed0bf5a.pdf" — the uploaded name never touches the filesystem path. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T17.1 | A job is created and queued | a job number is issued | Job created; payment_required=false, redirect=/print/success/958d07dfc5e3d6617f641283ec25f2e6 | **PASS** |
...
| T17.2 | The worker sends the job to the printer over IPP | the printer accepts it | Status "completed", printer job id 10. Worker output: 20:12:50 [sent] AK100001 — Printer accepted the job (IPP job 10). \| 20:12:50 Reconciled: 1 completed, 0 failed of 1 checked \| 20:12:50 Worker finished: 1 job(s) in 0 second(s) | **PASS** |
...
| T10.1 | A4 printing works end to end | the job completed on A4 | Completed on A4 at 2026-09-13 20:12:50 UTC. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T25.1 | A database backup is created and is restorable | a verified, non-trivial dump | Backup db-20260913-201251-b3a34959.sql.gz: 12,113 bytes, checksum verified, contains CREATE TABLE for print_jobs. | **PASS** |
| T25.2 | An application backup is created | a zip containing the code | Application backup app-20260913-201252-85d84f0d.zip contains the code and correctly excludes customer uploads. | **PASS** |
...
| T29.3 | The QR code is genuinely scannable | decodes back to the print URL | Version 7 QR (45×45 modules) rendered as a 2,388 byte PNG. Independently decoded with OpenCV during development. | **PASS** |
...
| T30.4 | Health checks across 50+ printers complete promptly | a bounded batch | Completed in 49 ms — 20:12:54 [printer_health] checked 0 — 0 online, 0 not (3 ms) | **PASS** |
...
| TG.2 | The agent handles its options, its printers and its own config correctly | every agent check passes | 85 passed, 0 failed | **PASS** |
...
| TD.1 | The agent API registers a printer for the agent that asked | created once, scoped to that device | Created as desk-printer-3ab2 on the agent transport, bound to its own device and location; re-registering updated it instead of adding a second printer. | **PASS** |
| TD.2 | The desktop window connects, lists printers and registers one | driven for real, headlessly | 73 passed, 0 failed | **PASS** |
...
| TP.5 | The shop signs in and issues its own agent token | a working token, nobody read it aloud | Token kpms_A2HbfBH… works against the agent API as "Ramesh's Xerox & Stationery counter", is stored only as a SHA-256 hash, and is not shown again on the next page load. | **PASS** |
...
_Generated by `tests/run.php` on 2026-09-13 20:13:01 UTC. Re-run with `php tests/run.php`._
