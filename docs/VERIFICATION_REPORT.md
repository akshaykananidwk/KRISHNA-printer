...
| | |
|---|---|
| Run at | 2026-09-13 14:06:52 UTC |
...
| T23.5 | Secrets are encrypted at rest | ciphertext, not the plaintext value | Stored as sod1:JRPFoB1… — the plaintext does not appear in the database. | **PASS** |
...
| T2.2 | The QR token is stored only as a hash | no plaintext token in the database | Only the SHA-256 hash is stored (hint: 58950462…). A stolen database dump yields no working QR link. | **PASS** |
...
| T5.4 | Stored files get a random name, not the uploaded one | no original filename on disk | Original "twelve-pages.pdf" stored as "d9e79c16f54b0b3016374d58b71c74bfbf92de89.pdf" — the uploaded name never touches the filesystem path. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T17.1 | A job is created and queued | a job number is issued | Job created; payment_required=false, redirect=/print/success/4df8564cd8d932e477f239dfc436f5fd | **PASS** |
...
| T17.2 | The worker sends the job to the printer over IPP | the printer accepts it | Status "completed", printer job id 5. Worker output: 14:06:47 [sent] AK100001 — Printer accepted the job (IPP job 5). \| 14:06:47 Reconciled: 1 completed, 0 failed of 1 checked \| 14:06:47 Worker finished: 1 job(s) in 0 second(s) | **PASS** |
...
| T10.1 | A4 printing works end to end | the job completed on A4 | Completed on A4 at 2026-09-13 14:06:47 UTC. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T25.1 | A database backup is created and is restorable | a verified, non-trivial dump | Backup db-20260913-140648-c4f59151.sql.gz: 11,758 bytes, checksum verified, contains CREATE TABLE for print_jobs. | **PASS** |
| T25.2 | An application backup is created | a zip containing the code | Application backup app-20260913-140649-c800d66d.zip contains the code and correctly excludes customer uploads. | **PASS** |
...
| T30.4 | Health checks across 50+ printers complete promptly | a bounded batch | Completed in 52 ms — 14:06:51 [printer_health] checked 0 — 0 online, 0 not (2 ms) | **PASS** |
...
_Generated by `tests/run.php` on 2026-09-13 14:06:52 UTC. Re-run with `php tests/run.php`._
