...
| | |
|---|---|
| Run at | 2026-09-13 19:23:47 UTC |
| Duration | 17 seconds |
| PHP | 8.4.25 |
...
**129 passed · 0 failed · 0 skipped** of 129 checks.
...
| T23.5 | Secrets are encrypted at rest | ciphertext, not the plaintext value | Stored as sod1:WZRSMc5… — the plaintext does not appear in the database. | **PASS** |
...
| T2.2 | The QR token is stored only as a hash | no plaintext token in the database | Only the SHA-256 hash is stored (hint: 8e3c8759…). A stolen database dump yields no working QR link. | **PASS** |
...
| T5.4 | Stored files get a random name, not the uploaded one | no original filename on disk | Original "twelve-pages.pdf" stored as "7e90a51262a5c07f0dc8f4f8428d5535d1e99acf.pdf" — the uploaded name never touches the filesystem path. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T17.1 | A job is created and queued | a job number is issued | Job created; payment_required=false, redirect=/print/success/c4997b4afa125c51b110b91985aed022 | **PASS** |
...
| T17.2 | The worker sends the job to the printer over IPP | the printer accepts it | Status "completed", printer job id 11. Worker output: 19:23:35 [sent] AK100001 — Printer accepted the job (IPP job 11). \| 19:23:35 Reconciled: 1 completed, 0 failed of 1 checked \| 19:23:35 Worker finished: 1 job(s) in 0 second(s) | **PASS** |
...
| T10.1 | A4 printing works end to end | the job completed on A4 | Completed on A4 at 2026-09-13 19:23:35 UTC. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T25.1 | A database backup is created and is restorable | a verified, non-trivial dump | Backup db-20260913-192337-7ace4a36.sql.gz: 12,117 bytes, checksum verified, contains CREATE TABLE for print_jobs. | **PASS** |
| T25.2 | An application backup is created | a zip containing the code | Application backup app-20260913-192338-70ed3b91.zip contains the code and correctly excludes customer uploads. | **PASS** |
...
| T29.3 | The QR code is genuinely scannable | decodes back to the print URL | Version 7 QR (45×45 modules) rendered as a 2,406 byte PNG. Independently decoded with OpenCV during development. | **PASS** |
...
| T30.4 | Health checks across 50+ printers complete promptly | a bounded batch | Completed in 50 ms — 19:23:40 [printer_health] checked 0 — 0 online, 0 not (2 ms) | **PASS** |
...
| TG.2 | The agent handles its options, its printers and its own config correctly | every agent check passes | 83 passed, 0 failed | **PASS** |
...
| TD.1 | The agent API registers a printer for the agent that asked | created once, scoped to that device | Created as desk-printer-5d93 on the agent transport, bound to its own device and location; re-registering updated it instead of adding a second printer. | **PASS** |
| TD.2 | The desktop window connects, lists printers and registers one | driven for real, headlessly | 69 passed, 0 failed | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| TP.1 | Anyone can register a shop from the public site | a registration, and nothing more | Recorded as "Ramesh's Xerox & Stationery", waiting for approval. Locations still 52 and print agents still 4 — registering creates neither. | **PASS** |
...
| TP.5 | The shop signs in and issues its own agent token | a working token, nobody read it aloud | Token kpms_v2c-E1A… works against the agent API as "Ramesh's Xerox & Stationery counter", is stored only as a SHA-256 hash, and is not shown again on the next page load. | **PASS** |
...
_Generated by `tests/run.php` on 2026-09-13 19:23:47 UTC. Re-run with `php tests/run.php`._
