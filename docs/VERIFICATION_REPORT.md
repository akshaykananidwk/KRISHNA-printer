...
| | |
|---|---|
| Run at | 2026-09-14 14:09:24 UTC |
...
**130 passed · 0 failed · 0 skipped** of 130 checks.
...
| T23.5 | Secrets are encrypted at rest | ciphertext, not the plaintext value | Stored as sod1:zheX+PA… — the plaintext does not appear in the database. | **PASS** |
...
| T13.2 | The connection test reaches the printer | reports online | Online over ipp in 7 ms: Ready to print. | **PASS** |
...
| T2.2 | The QR token is stored only as a hash | no plaintext token in the database | Only the SHA-256 hash is stored (hint: facf46ee…). A stolen database dump yields no working QR link. | **PASS** |
...
| T5.4 | Stored files get a random name, not the uploaded one | no original filename on disk | Original "twelve-pages.pdf" stored as "f05f1ff2abb295c782413439cf86cee8e387e962.pdf" — the uploaded name never touches the filesystem path. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T17.1 | A job is created and queued | a job number is issued | Job created; payment_required=false, redirect=/print/success/334ef9b4713c45ae83748ae7a6e563c9 | **PASS** |
...
| T17.2 | The worker sends the job to the printer over IPP | the printer accepts it | Status "completed", printer job id 1. Worker output: 14:09:11 [sent] AK100001 — Printer accepted the job (IPP job 1). \| 14:09:11 Reconciled: 1 completed, 0 failed of 1 checked \| 14:09:11 Worker finished: 1 job(s) in 0 second(s) | **PASS** |
...
| T10.1 | A4 printing works end to end | the job completed on A4 | Completed on A4 at 2026-09-14 14:09:11 UTC. | **PASS** |
...
| # | Test | Expected | Actual | Result |
|---|------|----------|--------|--------|
| T25.1 | A database backup is created and is restorable | a verified, non-trivial dump | Backup db-20260914-140913-2d674ce6.sql.gz: 12,122 bytes, checksum verified, contains CREATE TABLE for print_jobs. | **PASS** |
| T25.2 | An application backup is created | a zip containing the code | Application backup app-20260914-140914-4f64ecae.zip contains the code and correctly excludes customer uploads. | **PASS** |
...
| T29.3 | The QR code is genuinely scannable | decodes back to the print URL | Version 7 QR (45×45 modules) rendered as a 2,413 byte PNG. Independently decoded with OpenCV during development. | **PASS** |
...
| T30.2 | Dashboard aggregates stay fast at scale | under 500 ms | Dashboard rendered in 12 ms with 50+ locations and printers. | **PASS** |
...
| T30.4 | Health checks across 50+ printers complete promptly | a bounded batch | Completed in 65 ms — 14:09:17 [printer_health] checked 0 — 0 online, 0 not (2 ms) | **PASS** |
...
| TD.1 | The agent API registers a printer for the agent that asked | created once, scoped to that device | Created as desk-printer-2c70 on the agent transport, bound to its own device and location; re-registering updated it instead of adding a second printer. | **PASS** |
...
| TP.5 | The shop signs in and issues its own agent token | a working token, nobody read it aloud | Token kpms_H0cEkfH… works against the agent API as "Ramesh's Xerox & Stationery counter", is stored only as a SHA-256 hash, and is not shown again on the next page load. | **PASS** |
...
_Generated by `tests/run.php` on 2026-09-14 14:09:24 UTC. Re-run with `php tests/run.php`._
